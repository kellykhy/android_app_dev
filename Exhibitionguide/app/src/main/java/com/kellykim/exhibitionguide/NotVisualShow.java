...
public class NotVisualShow extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_notvisualshow);
        ...
        exhibition = intent.getExtras().getString(EXHIBITION);
        ...
        timeArray1 = getResources().getStringArray(R.array.array1);

        if ((now_hour >= 9 && now_hour <= 19) || (now_hour == 19 && (now_min <= 30))) {
                if (now_min >= 30 && now_min <= 59) {
                    now_hour += 1;
                    default_time = (now_hour - 9) * 2;
                } else {
                    default_time = (now_hour - 9) * 2 + 1;
                }
            } else {
                default_time = 0;
            }
            ArrayAdapter<String> adapter = new ArrayAdapter<>(
                    this, android.R.layout.simple_spinner_item, timeArray1
            );
    ...
    public boolean onOptionsItemSelected (MenuItem item){
        ...
        if (id == R.id.menu_select_info) {
            ...
            intent1.putExtra(EXHIBITION, exhibition);
